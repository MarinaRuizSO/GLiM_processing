...
def plot_glim_with_basins(axs, DEM_path, DEM_file, location_name, save_path, hs_raster):
    print(f'I am processing {location_name}')

    lon_transform_list = []
    lat_transform_list = []
    filelist = count_outlets_in_mountain(DEM_path)
    count_file_list = 0
    ...
    # need to supply a list of files in the format "<LOCATION_NAME>_basin_<X>.csv" with a latitude and a longitude column
    # for the position of the outlets of each basin
    for file in filelist[:]:
        ...
        dem_basins_with_rain = lsd.LSDDEM(path = DEM_path,file_name = DEM_file)
        ...
        dem_basins_with_rain.ksn_MuddEtAl2014(target_nodes=30, n_iterations=60, skip=1, nthreads = 1)
        df_basins_with_rain_new = dem_basins_with_rain.df_ksn
        ...
        add_basin_outlines(dem_basins_with_rain, axs, size_outline = 1, zorder = 4, color = "purple")

        my_dem_raster = lsd.raster_loader.load_raster(DEM_path+DEM_file)
    ...
    plt.savefig(save_path+f'{location_name}_lithoGLim_all_basins_TEST_river.png', dpi=500, bbox_inches='tight')

def plot_glim_no_basins(axs, DEM_path, DEM_file, location_name, save_path, hs_raster):
    print(f'I am processing {location_name}')
